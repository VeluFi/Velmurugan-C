...
import { Github, Star, GitFork, ExternalLink, Code2, Sparkles, Loader2 } from "lucide-react";
...
export const GitHubSection: React.FC = () => {
  ...
  return (
    <section id="github" className="py-20 bg-slate-950/70 border-t border-slate-900">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-12 border-b border-slate-800 pb-8">
          ...
          {/* GitHub Stats Pill */}
          {profile && (
            <div className="flex items-center gap-4 p-3 rounded-xl bg-slate-900 border border-slate-800 font-mono text-xs text-slate-300">
              <div>
        ...
        {loading ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {repos.map((repo) => (
              <div
                key={repo.id}
                className="bg-slate-900/60 rounded-2xl p-6 border border-slate-800/80 hover:border-sky-500/40 transition-all flex flex-col justify-between space-y-4 group glass-card-hover"
              >
        ...
        <div className="mt-12 text-center">
          <a
            ...
            target="_blank"
            rel="noopener noreferrer"
            className="inline-flex items-center gap-2 px-6 py-3 rounded-xl bg-slate-900 hover:bg-slate-800 border border-slate-800 text-white font-medium text-sm transition-colors shadow-lg"
          >
